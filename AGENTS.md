...
独立的 Node.js 子项目，不参与桌面端/Web 端构建。把「剧本 → 角色 → 场景 → 分镜 → S 级视频」全流程暴露为 MCP tools + prompts，让 Codex Desktop / Cursor / Codex 能直接驱动视频生成。设计上**故意不复用 `src/lib`**（那里强依赖 Zustand store + Electron preload），改为：
